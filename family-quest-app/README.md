...
- 실제 Supabase 프로젝트 설정(1장)·배포(3장)·푸시 알림 설정(3-1장)·계정 2개 실사용 테스트(5장)·RLS 보안 테스트(6장)는 모두 완료되어, 지금은 실제 배포된 앱을 가족이 일상적으로 쓰면서 나오는 피드백을 바탕으로 계속 수정·기능 추가가 이루어지는 단계입니다. 다만 이 코드 작업을 하는 개발 환경 자체는 외부 네트워크가 제한되어 있어, AI가 직접 그 Supabase 프로젝트나 배포된 사이트에 접속해 확인하지는 못합니다 — `supabase/schema.sql` 변경 사항은 로컬 Postgres 16 인스턴스에 적용해 함수/트리거/정책 동작을 검증하고, UI 변경은 정적 렌더링(Playwright)으로 검증한 뒤, 실제 반영 여부는 사용자 피드백(스크린샷 등)으로 확인하는 방식으로 진행합니다.
