...
- **[2026-09-09, 감독 확정] 장기 비주얼 방향 격차 — `docs/UI_VISUAL_DIRECTION_REQUEST.md`
  참고**: 감독이 목업 3장("카드 컬렉션", "홈/상세/공유카드", "온보딩/컴포넌트")을
  전달, 지금 앱과의 격차를 정리해 디자인팀 레퍼런스용 문서로 만듦. 핵심 격차는
  사건 타입 14종 전용 일러스트 부재(지금은 캐릭터 포즈 6~7종 재사용), 등급 뱃지가
  알약형 아이콘+라벨이 아님, 브랜드 태그라인 미사용, 온보딩 전용 일러스트 부재.
  감독 확정: 이 항목보다 우선순위 A/B(실기기 버그 수정)가 먼저 — 지금 당장 작업
  아님, 디자인팀에 전달해두고 회신 오면 순서대로 반영.
