...
- **[2026-09-09, 감독 확정, 회신 대기] 사건 일러스트 9종 재작업 요청** — 보관함
  화면에서 overlay 타입 9종(5초컷/재입장 사건/원점 회귀/목적불명 순찰/탈출 실패/
  오늘의 첫 상대/심야 순찰/100회 방문/디지털 미아)이 scene 타입 5종에 비해
  부실해 보인다는 지적 → v0.29(배경 사진 추가)/v0.30(캐릭터 확대 크롭)까지
  코드로 완화 시도했지만 감독 판단으로는 부족 — "제대로 된 일러스트가 아니면
  재미없다"고 확정. 9종 전부 scene 타입처럼 완성된 배경 일러스트로 다시 그려달라고
  디자인팀에 정식 요청함(`docs/ASSET_REQUESTS_FOR_DESIGN.md` 6번). 회신 오면
  v0.29/v0.30의 임시 코드(overlay 배경 합성, `overlay-fill` 크롭)는 자연스럽게
  걷어낼 수 있음.
...
- ~~앱 온보딩 완성~~ — v0.24에서 완료. 디자인팀 온보딩 일러스트(1/2, 2/2 × KO/JP)를
  ...
  기능이 없어서 fire-and-forget — 알림 기능을 실제로 만들 때 이 권한이 이미
  있다고 가정하지 말고 상태를 확인할 것.
