...
- ~~[2026-09-09, 감독 확정, 범위 재확대] 사건 일러스트 전체
  ...
  **v0.32에서 완료.** 보관함 화면에서 overlay 타입 9종이 scene 타입 5종에
  ...
  남아있지만 더는 어디서도 타지 않음.
  **남은 것**: 새 PNG 50장이 평균 ~1.5MB(장당)로 `incidents/card_ready/`
  용량이 91MB까지 늘어남 — APK 크기에 영향이 크므로 WebP 변환이나 해상도
  축소(1672x941 → 1200x675, GPT가 스펙보다 크게 뽑아준 원본) 압축 여부는
  감독 확인 후 별도 커밋으로 처리. 라벨 있는 UI 아이콘 10종은 자산만
