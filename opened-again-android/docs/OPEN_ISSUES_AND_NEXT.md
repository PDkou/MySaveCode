...
- v0.68 — 오늘의 사건 리빌 카드 애니메이션이 실기기에서 안 움직인다는
  리포트. `.tcg-card.reveal-anim`에 `.tilting`과 같은 `transition:none`
  을 추가해뒀지만, 이 샌드박스(헤드리스 크로미움)에서는 수정 전에도
  이미 정상 재생돼서 실기기 증상을 그대로 재현하지는 못했음 — 진짜
  원인인지 다음 실기기 확인 때 재확인 필요. 안 고쳐졌다면 카드가
  정확히 어떻게 보이는지(처음부터 완성된 크기로 바로 나타났는지,
  뒤집힌 채로 멈춰 있었는지, 안드로이드 "애니메이션 제거" 접근성
  설정이 켜져 있는지)를 추가로 확인해야 진짜 원인을 좁힐 수 있음.
