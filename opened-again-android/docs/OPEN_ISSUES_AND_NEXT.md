...
- ~~v0.75 — 광고 위 빈 여백 수정(`webView.requestLayout()` 명시 호출)이
  실제로 그 간격을 없애는지 실기기 재확인 필요~~ — 감독이 실기기에서
  재확인해보니 여전히 여백이 남아있었음(v0.73/v0.75는 각각 실제
  다른 버그를 고친 것이었지 이 여백의 진짜 원인은 아니었음). v0.77에서
  세 번째 시도로 진짜 원인(WindowInsets가 배너의 존재를 고려하지
  않고 WebView에도 같은 inset을 중복 전달)을 찾아 재수정 — 아래 v0.77
  항목 참고. 리빌 카드 드래그 미동작 쪽은 Playwright로 실제 원인(재현
  전: computed transform 고정 / 재현 후: 정상 반영)까지 확인해서 v0.68
  항목과 함께 해결 완료로 표시함(위 참고, 이쪽은 재확인 불필요).
...
- **v0.77 — 광고 위 빈 여백, 세 번째 수정 — 실기기 최종 확인
  최우선 필요**: 이번엔 원인을 "WebView가 배너의 존재를 모르고
  내비게이션 바 회피용 여백을 중복으로 예약한다"는 것으로 특정하고,
  네이티브가 계산한 정확한 값을 JS에 직접 밀어주는 방식(`--native-nav-bottom`
  CSS 커스텀 프로퍼티)으로 고쳤음. Playwright로는 이 값이 CSS에
  올바르게 반영되는 것까지만 확인 가능하고, 안드로이드가 실제로 어떤
  inset 값을 언제 전달하는지는 실기기에서만 확인 가능 — v0.73/v0.75도
  이론상 맞아 보였지만 실기기에서 여백이 남아있었던 전례가 있어서
  이번에도 확정은 실기기 재확인 후로. 만약 이번에도 남아있다면
  스크린샷 + 대략적인 여백 크기(px) + 어느 화면(기록/보관함/종료
  모달)인지를 같이 알려주면 원인을 훨씬 빨리 좁힐 수 있음.
