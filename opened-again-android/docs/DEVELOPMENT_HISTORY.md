...
## v0.56 — v0.55의 "앱 목록 직행" 시도가 실기기 회귀를 일으켜 되돌림
디렉터가 v0.55를 실기기에서 써보고: "처음 시작할때 사용정보 접근
허용을 눌러도 그 화면으로 안가고 앱이 시작되고 다시한번 허용을
눌러야되는 불편함이 없지않아 있음".

원인은 v0.55에서 같이 넣었던 두 가지 대응 중 첫 번째("`package:`
data URI로 앱 자신의 토글 화면 직행 시도")로 추정. 그 데이터 URI를
지원하지 않는 기기에서 인텐트가 던지는 예외를 잡아 원래 방식으로
폴백하도록 짰었는데, 실제로는 지원하지 않는 기기가 예외를 던지는
대신 그 인텐트를 (제대로 동작하지 않는 채로) 조용히 소비하고 바로
앱으로 되돌아오는 경우가 있었던 것으로 보임 -- `startActivity()`는
호출이 "성공"했는지만 알려줄 뿐 실제로 의도한 화면이 떴는지는
보장하지 않기 때문에, `runCatching`의 폴백이 아예 발동하지 않고
"버튼을 눌렀는데 아무 일도 안 일어난 것처럼 보이는" 상태가 됨 --
사용자 입장에서는 정확히 "그 화면으로 안 가고 앱이 시작되고 다시
눌러야 하는" 것으로 체감됨.

미확인/비공식 동작에 기댄 최적화라 이번엔 안전하게 되돌림 --
`package:` data URI를 완전히 제거하고 원래의 평범한
`Settings.ACTION_USAGE_ACCESS_SETTINGS` 단순 인텐트(모든 기기에서
보장되는 표준 동작)만 사용. v0.55의 나머지 절반(네이티브 Toast로
"또 열었네?"를 찾아 켜달라고 안내하는 것)은 그대로 유지 -- 이건
직행 여부와 무관하게 항상 안전하게 동작하는 부분이라 남겨둠.

버전 56/0.56.0. 이번에도 Android 프레임워크 코드라 이 세션에서
직접 실행 검증은 불가 -- 되돌린 변경 자체는 v0.54 이전까지 오래
쓰이던 코드와 동일한 한 줄이라 리스크가 낮다고 판단했고, CI
컴파일 통과로 확인. 실제 개선 여부는 실기기 재확인 필요.

## v0.57 — 온보딩 알림 허용 → 사용정보 접근, 두 번 안 물어보게 이어붙임
디렉터가 온보딩 "알림 허용" 화면과 홈 화면의 "사용정보 접근 허용"
화면 스크린샷 두 장을 같이 보내며: "첫번째화면에서 눌렀는데
두번째 사진에서도 또 눌러야된다는거야 그냥 통합하면되는데".
v0.55/v0.56에서 다룬 "사용정보 접근 목록에서 앱 찾기" 피드백과는
결이 다른, 별개의 실제 문제였음 -- 두 권한 요청이 완전히 분리된
두 화면(온보딩 마지막 스텝 / 온보딩 끝난 뒤 홈 탭 진입 시 뜨는
별도 게이트)이라 사용자가 두 번 따로 눈치채고 눌러야 했던 것.

안드로이드에는 알림 권한(런타임 다이얼로그)과 사용정보 접근 권한
(설정 앱 화면에서만 부여 가능)을 한 번에 묻는 시스템 다이얼로그가
없어서 완전히 한 번의 탭으로 합칠 수는 없음. 대신 `onboardFinish()`
가 온보딩을 끝내는 시점에 `N.hasUsageAccess()`로 사용정보 접근이
아직 없으면 바로 이어서 `N.openUsageSettings()`까지 호출하도록 체이닝
-- "알림 허용"이든 "나중에"든 상관없이(사용정보 접근은 알림과 달리
앱이 동작하는 데 필수라 알림 선택과 무관하게 필요하면 이어붙임),
사용자가 홈 화면에서 별도 게이트를 다시 찾아 누를 필요 없이 온보딩의
자연스러운 연장으로 이어짐. 이미 권한이 있으면(재실행/이미 허용한
경우) 당연히 다시 부르지 않음.

버전 57/0.57.0. Playwright로 `window.OpenedAgainNative`(index.html이
`N`으로 캡처하는 실제 전역 이름 -- `window.N`이 아님, 처음엔 이걸로
착각해서 테스트가 거짓으로 통과한 걸 한 번 잡아냄) 스텁을
`addInitScript()`로 페이지 스크립트 실행 전에 주입해서 3가지 경우
모두 확인: (1) 사용정보 접근 미허용 + "알림 허용" 탭 → 알림 권한
요청과 `openUsageSettings()` 둘 다 호출됨, (2) 이미 허용된 상태에서
"나중에" 탭 → 둘 다 호출 안 됨(중복 호출 없음), (3) N이 아예 없는
프리뷰 모드 → 에러 없이 온보딩 정상 종료. 기존 상세 페이지/뒤로가기
회귀 스위트도 재확인, 콘솔 에러 0건.

## v0.58 — 사용정보 접근 목록에서 "우리 앱 위치 표시" 요청, 플랫폼 한계 확인 후 대안 적용
디렉터: "그리고 이제 수많은 앱들중에 우리 앱이 어디있는지 알려주는
것까지 기능으로 넣으면 좋겟지" -- v0.55/v0.56의 "이름을 찾아라"
토스트 안내만으로는 부족하다는 후속 요청.

실제로 뭐가 가능한지 검토함: 안드로이드는 앱 샌드박싱 때문에 우리
앱에서 시스템 설정 앱(사용정보 접근 목록) 화면 위에 화살표나
하이라이트를 그려서 "여기예요"라고 가리키는 게 원천적으로 불가능함.
그러려면 다른 앱 위에 그리기(`SYSTEM_ALERT_WINDOW`) 권한 + 화면
요소를 찾아내는 접근성 서비스(`AccessibilityService`)가 둘 다
필요한데, 둘 다 이 기능 하나를 위해 쓰기엔 지나치게 무겁고
사생활 침해 소지가 크며, 플레이스토어 정책상 접근성 서비스를
비접근성 목적(이런 용도)으로 쓰는 건 명확히 금지/반려 대상임 --
무엇보다 "권한 요청을 줄이자"는 이번 v0.55~v0.57의 취지 자체와
정반대로 권한을 하나 더 요구하게 되는 모순이 생김. 그래서 이건
새 권한을 추가하지 않는 선에서는 넘을 수 없는 실제 플랫폼 한계로
판단하고, 그 안에서 할 수 있는 걸 개선함.

`NativeBridge.openUsageSettings()`의 안내 토스트에 "검색 아이콘이
있다면 이름으로 검색해보세요" 문구를 추가 -- 삼성 One UI의 "특별
접근" 목록을 포함해 여러 안드로이드 스킨이 이 화면에 실제로 검색
기능을 제공하는데, 있으면 목록을 눈으로 훑는 것보다 훨씬 빠르게
찾을 수 있고 없는 기기에서도 그냥 무시되니 손해가 없음.

버전 58/0.58.0. 문자열만 바꾼 네이티브 코드 변경이라 CI 컴파일
통과로 확인 -- 실제로 검색 아이콘이 있는 기기/롬 조합인지, 안내가
충분히 도움이 되는지는 실기기 확인 필요. 디렉터에게는 플랫폼 한계
자체를 있는 그대로 설명하고, "완전히 가리키는 기능"은 불가능하다는
점을 솔직하게 전달함.

## v0.59 — Pocket 효과가 아트창에만 갇혀 있던 걸 카드 전체로 확장
디렉터가 실기기에서 확인: "카드가 3d처럼 움직이는 것을 확인했는데
일러스트있는 부분을 드래그해야 움직이는 것을 확인했어 카드 전체가
드래그 되게 하고 프리즘 효과도 카드 전체에 되게 해야된다고 생각해".
v0.54에서 카드 3D 기울임 자체는 잘 작동했지만, 드래그 감지 영역과
홀로그램 캔버스 둘 다 여전히 `.tcg-art`(일러스트 창) 안에만 갇혀
있었던 게 원인.

**구조 변경**: `.holo-canvas`를 `.tcg-art`의 자식에서 `.tcg-card`
자체의 (마지막) 자식으로 옮김. `mountHolo()`의 모든 드래그 감지/기울임
로직이 `scene`(=`canvas.parentElement`)을 기준으로 동작하도록 이미
범용적으로 짜여 있었기 때문에, 캔버스가 어디 붙는지만 바꾸면 드래그
감지 영역과 캔버스 크기 둘 다 자동으로 카드 전체로 넓어짐 -- JS
로직 자체는 변경 불필요.

**캔버스 알고리즘 변경**: 캔버스가 이제 카드 전체(프레임 배경 +
헤더 패널 + 아트 사진 + 정보 패널 + 메달)를 덮게 됐는데, 기존
`createCardHolo()`는 캔버스 안에 사진을 직접 다시 그려 넣고 그 위에
색/스크린 블렌드로 홀로그램을 합성하는 방식이었음 -- 카드 전체를
캔버스에 다시 그리려면 사실상 카드 렌더링 전체를 Canvas 2D로
재구현해야 해서 배보다 배꼽이 커짐. 대신 캔버스를 완전히 투명한
오버레이로 바꾸고(사진/프레임 등 어떤 것도 다시 그리지 않음), CSS
`mix-blend-mode:screen`을 캔버스 엘리먼트 자체에 걸어서 브라우저가
알아서 진짜 카드 내용(등급 무관하게 프레임/패널/아트/메달 전부)과
합성하도록 함 -- 기존에도 가장 두드러진 레이어들(스펙트럼 2차 패스,
글레어, 림 라이트)이 이미 'screen' 블렌드를 쓰고 있었어서 전체
캔버스를 screen으로 바꿔도 전체적인 느낌은 유지됨. 이 변경으로
`image`/`makeCanvas`의 커버-핏 크롭 로직 전체가 필요 없어져서
`createCardHolo()`가 꽤 단순해짐(이미지 로딩 대기도 필요 없어져서
`mountHolo()`도 `img.onload` 콜백 없이 즉시 시작하도록 단순화).

버전 59/0.59.0. Playwright로 확인: `.holo-canvas`의 부모가 이제
`.tcg-card`이고 캔버스 크기가 정확히 카드 전체 크기와 일치, 헤더
패널 위에서 시작한 드래그와 정보 패널 위에서 시작한 드래그 둘 다
`.tcg-card`의 `transform`이 `matrix3d(...)`로 바뀌는 것(아트창이
아닌 다른 영역에서 드래그해도 카드 전체가 기울어짐) 확인,
`mix-blend-mode:screen`이 실제로 적용된 것 확인. 기존 5개 등급
전체 화면 검증/스크롤 없음/뒤로가기/온보딩 이어붙임 회귀 스위트도
전부 재확인, 콘솔 에러 0건.

## v0.60 — 등급별 프리즘 색상 + Pocket 스타일 "오늘의 사건 발견!" 리빌 화면
디렉터가 v0.59를 실기기에서 확인하고 만족("카드가 잘 기울여지는거
너무좋아") 후 두 가지 추가 요청:
1. "레전더리는 금색 에픽은 보라색으로 프리즘효과를 주고 히든은
   레인보우로 효과를 주는게 더 좋을꺼같아"
2. "저녁9시?에 통계로 알려줄때 앱을 열때 가장 많이 접근된 카드를
   뙇하고 보여주면 더 좋을꺼같아 어떤어떤 사건 발견됨! 하면서 카드가
   휘리릭 돌아가면서 ... 대충 무슨느낌인지 알겠니? 모르겠으면 아까
   참고한 pocket을 참고해도 될듯"

**1. 등급별 프리즘 색상**: `createCardHolo()`의 무지개 스펙트럼
그라디언트가 지금까지 모든 등급에 똑같은 팔레트 하나를 썼음 --
새 `holoSpectrum(rarity)` 함수가 LEGENDARY(금색 계열), EPIC(보라
계열), HIDDEN(레인보우) 각각 다른 7색 팔레트를 반환하도록 분리.
HIDDEN은 새 팔레트를 따로 만들지 않고 카드 타이틀 foil 효과가 이미
쓰고 있는 `foilTitleStyle('HIDDEN').colors`를 그대로 재사용해서
타이틀과 카드 전체 홀로그램이 항상 같은 무지개를 쓰도록 함. 색상
배열은 캔버스 엘리먼트의 `data-spectrum` 속성으로 전달(mountHolo()
자체는 여전히 등급을 몰라도 되는 범용 함수로 유지). 이 김에 HIDDEN도
"impact"(홀로그램 적용) 등급에 포함시킴 -- v0.27 당시엔 HIDDEN이 이미
전용 다크 테마가 있다고 제외했었지만, 이제 전용 무지개 팔레트까지
생기면서 그 판단을 뒤집을 근거가 명확해짐.

**2. "오늘의 사건 발견!" 리빌 화면**: 하루 최초로(달력 날짜 기준
1회, `state.history.revealShownDate`) 앱을 열었을 때, 오늘 리포트의
"대표" 사건(`cases()`가 이미 "featured"로 취급하는 것과 동일하게
`cards[0]`)을 Pocket 카드팩 개봉 연출로 극적으로 보여주는 새 화면
(`revealScreen()`). `detailPage()`의 `.tcg-card` 마크업/홀로그램
효과를 그대로 재사용(새 시각 시스템을 또 만들지 않음)하고, 카드
자체에 "face-down에서 뒤집히며 확대되는" 1회성 스핀인 애니메이션
(`.reveal-anim`)과 원형 플래시 버스트(`.reveal-flash`)를 추가. 뒤로가기
버튼도 다른 전체화면 오버레이들과 동일하게 이 화면을 닫도록 연결.

**구현 중 발견한 버그**: `fitTcgCard()`/`mountHolo()`가 카드 크기를
잴 때 `getBoundingClientRect()`를 쓰고 있었는데, 이건 CSS
`transform`(리빌의 스핀인 애니메이션이나 드래그 중 기울임)이 적용된
"현재 보이는" 크기를 반환함 -- 리빌 애니메이션이 아직 진행 중인
순간(스케일 40%)에 이 값으로 카드 크기를 재면 실제 레이아웃 크기가
아니라 일시적으로 쪼그라든 값을 기준으로 잘못된 크기 계산을 할 위험이
있었음. 둘 다 `offsetWidth`/`offsetHeight`(transform에 영향받지 않는
실제 레이아웃 크기)로 교체해서 해결 -- 리빌 기능이 없었어도 잠재적
문제였을 부분이라 일반적인 견고성 개선이기도 함.

**테스트용 이스케이프 해치 추가**: 기존 `?onboarding=0`과 같은 패턴으로
`?reveal=0`을 추가 -- 로컬스토리지가 없는 새 프리뷰/테스트 로드마다
리빌이 매번 무장되면서 그 아래 실제 사건 상세/홈 화면을 겨냥한 기존
Playwright 테스트들을 전부 가려버렸기 때문(실서비스 동작에는 영향 없음,
테스트 전용 파라미터).

버전 60/0.60.0. Playwright로: 등급별 `data-spectrum` 값이 실제로
다른 것과 화면 스크린샷으로 색감 차이 확인, 리빌 화면이 첫 로드에
무장되고 카드가 완전한 크기(애니메이션 종료 후 `transform:none`)로
자리잡는 것, "확인" 버튼으로 닫으면 `revealShownDate`가 오늘 날짜로
저장되고 같은 날 `refresh()`를 다시 불러도 재무장되지 않는 것까지
...
## v0.62 — "가장 오래 본 앱" 이름이 여전히 안 나오는 진짜 원인 (패키지 가시성 제한, 매니페스트 누락)
디렉터: "그 앱이름 보여주는거 말이야 ... 그건 알아보기 힘든 이름이
아니라 앱자체이름을 가지고 올 수는 없는거야?" -- v0.51에서 이미
`NativeBridge.appLabel()`(PackageManager로 실제 앱 이름 조회)을
만들어뒀는데도 디렉터 실기기에서는 여전히 안 되고 있었다는 뜻이라,
"코드는 맞는데 왜 실제로는 안 되는가"부터 다시 확인함.

원인은 안드로이드 11(API 30)부터 생긴 **패키지 가시성 제한
(package visibility)**: targetSdk 30 이상인 앱(이 앱은 36)이
`PackageManager.getApplicationInfo()`로 다른 앱을 조회하려면, 그
다른 앱이 매니페스트의 `<queries>`에 미리 선언돼 있거나 몇 가지
예외 조건에 해당해야 함. 아무 선언도 없으면 대부분의 실제 설치
앱 조회가 `NameNotFoundException`으로 실패 -- `appLabel()`은 이미
`runCatching{...}.getOrDefault("")`로 이 실패를 조용히 삼키고
있었고, `compactPkg()`도 결과가 비어 있으면 예전의 읽기 힘든
패키지명 기반 표시로 조용히 폴백하고 있었기 때문에, v0.51 코드
자체는 정상 동작인데도 매니페스트에 이 선언 하나가 빠져 있어서
실기기에서는 항상 폴백 경로만 타고 있었던 것.

전체 앱 조회 권한(`QUERY_ALL_PACKAGES`)은 플레이스토어 심사에서
별도 소명이 필요한 민감 권한이라 쓰지 않고, 구글이 이런 용도로
공식 권장하는 방식대로 `<queries><intent><action ...MAIN/><category
...LAUNCHER/></intent></queries>`를 추가 -- "홈 화면에 아이콘이
있는 앱들"(=사용자가 실제로 알아보는 앱 전부)만 조회 가능하게
열어주는, 최소 범위의 선언.

**주석 작성 중 또 한 번 잡은 실수**: 이번 주석 초안에도 `--`(더블
하이픈)를 구두점으로 세 군데 썼다가 v0.47과 똑같은 XML 주석 조기
종료 버그를 스스로 만들 뻔함 -- 이번엔 작성 직후 바로
`python3 -c "import xml.etree.ElementTree as ET; ET.parse(...)"` +
주석 본문에 리터럴 `--`가 있는지 정규식으로 검사하는 루틴을 돌려서
푸시 전에 잡아냄(`;`로 교체).

버전 62/0.62.0. 매니페스트/권한 선언만 바뀌는 변경이라 이 세션
도구로 실행 검증은 불가 -- XML 파싱 통과 확인 + CI 빌드 통과로
확인, 실제로 앱 이름이 제대로 나오는지는 실기기 확인 필요.

## v0.63 — 디렉터 요청으로 전체 훑어보기 (사건 상세 통계 라벨 미번역 버그 발견/수정, 백업 스키마 버전 추가)
디렉터: "또 따로 필요한거나 이런것도 있으면 더 좋겠다 라던가 수정할
것이라던가 등등 찾아보렴". `docs/OPEN_ISSUES_AND_NEXT.md`를 다시
훑어보고, 코드 전체에서 비슷한 종류의 버그가 더 있는지 능동적으로
찾아본 결과 두 가지 처리:

**1. 사건 상세 페이지 통계 라벨 미번역 버그 (발견)**: v0.38에서
`windowMs` 하나가 라벨 케이스 누락으로 원본 영문 키가 그대로
노출됐던 것과 같은 종류의 버그가 더 남아있는지, `IncidentDetector.kt`
가 실제로 만드는 모든 `metrics` 맵 키(`mapOf(...)`에 쓰인 키 전부)를
`metricLabel()`의 케이스와 전수 대조함. 실제로 5개나 더 있었음:
`afterUnlockMs`(FIRST_CONTACT), `avgStayMs`(APP_WANDERING),
`totalUsageMs`(HIDDEN_NIGHT_ACTIVITY), `visits`(RETURN_TO_START --
이번 세션 v0.53 작업 중 스크린샷으로 실제로 본 적 있었는데 그때는
범위 밖이라고 넘어갔던 바로 그 버그)는 케이스 자체가 아예 없었고,
`unlockSessions`(HIDDEN_NIGHT_ACTIVITY)는 케이스 이름이 `unlocks`로
잘못 붙어있어서 절대 매칭 안 되는 죽은 코드였음. 5개 다 추가/수정.
Playwright로 실제 13개 키 전부 넣어서 `metricLabel()` 결과를 확인(전부
번역된 라벨, 원본 키 그대로 노출되는 것 0건), RETURN_TO_START 카드
상세 페이지 스크린샷으로 "visits"가 "방문 횟수"로 바뀐 것도 직접 확인.

**2. 백업 스키마 버전 추가**: `docs/OPEN_ISSUES_AND_NEXT.md`에 오래
남아있던 항목. `save()`(로컬스토리지/네이티브 백업)와
`exportBackupData()`(설정 → 데이터 내보내기) 페이로드에
`schemaVersion:1`을 추가 -- `restore()`는 여전히 `settings`/`history`
만 읽으므로 기존 동작에 영향 없이, 앞으로 저장 형태가 바뀔 때
`restore()`가 버전을 보고 분기할 수 있는 발판만 마련.

**director 확인이 필요해서 코드로 처리 못한 것**: 설정의
개인정보처리방침 시트에 아직 남아있는 `[문의 이메일 주소를
입력하세요]`/`[출시 전 날짜를 입력하세요]` 자리표시자 -- 실제 연락처
이메일과 출시 예정일은 디렉터가 알려줘야 채울 수 있음(스토어 등록
전 필수).

버전 63/0.63.0. 두 항목 다 Playwright로 검증(신규 라벨 13개 전부
확인, 백업 스키마 버전 필드 저장/복원 정상 확인), 기존
회귀 스위트(뒤로가기/온보딩 이어붙임/상세 페이지 5개 등급) 전부
재확인, 콘솔 에러 0건. `docs/OPEN_ISSUES_AND_NEXT.md`도 두 항목
해결 처리로 갱신.

## v0.64 — 개인정보처리방침 자리표시자 채움 (감독 확인)
v0.63에서 플래그했던 개인정보처리방침 시트의 `[문의 이메일 주소를
입력하세요]`/`[출시 전 날짜를 입력하세요]` 자리표시자를 감독이 직접
확인해줌: "이메일은 헬로우 투데이와 같고 출시 전 날짜는 오늘날짜라도
상관없으면 그렇게 해줘".

- 문의 이메일: `db5704@gmail.com` -- 이 모노레포의 다른 앱
  "Hello, Today"(`hellotoday-app/`, `HelloToday` 브랜치)가 이미 자기
  개인정보처리방침/特定商取引法 표시에서 같은 용도로 쓰고 있는 주소인지
  직접 그 브랜치의 `index.html`을 열어 확인 후 그대로 재사용.
- 최종 수정일: 오늘 날짜(2026년 9월 14일 / 2026年9月14日 / September
  14, 2026, 3개 언어 각각 자연스러운 표기로).

한국어/일본어/영어 3개 버전 전부 반영, 코드 자체 주석도 "왜 이
값인지(감독 지시 + Hello Today와 동일)"를 남겨서 나중에 실제 법무
검토 시 근거를 알 수 있게 함. 이 문구 자체는 여전히 법무 검토를
거친 게 아니라는 경고는 그대로 유지 -- 자리표시자만 채워졌을 뿐 법적
검토 완료는 아님.

버전 64/0.64.0. Playwright로 시트를 열어서 대괄호 자리표시자가
하나도 안 남아있는 것, 이메일/날짜가 정확히 들어간 것 확인. 기존
...
## v0.66 — 사건 카드 설명을 실제 앱 이름이 들어간 서술형으로 개선 (감독 지시)
감독 피드백: "그 사건 카드들 설명이 너무 불친절한거같아 무슨앱을
들어갔는지 어떤일이일어나서 이걸얻었는지 자세히 알려줬음 좋겠음".

기존 `detail(i)`는 "5회의 짧은 실행이 발견됨"처럼 숫자만 있고 어떤
앱인지는 전혀 말하지 않았음 -- `card()`의 메타 스트립에 `i.primaryPackage`
필(앱 이름 pill)이 이미 따로 있긴 했지만, 설명 문장 자체와는 분리돼
있어서 "무슨 일이 일어났는지"를 한 문장으로 읽을 수 없었음.

**1. 앱 이름이 이미 있던 7개 사건 타입** (QUICK_EXIT/REENTRY/
ESCAPE_FAILED/REGULAR/HUNDRED_VISITS/FIRST_CONTACT/RETURN_TO_START --
전부 `IncidentDetector.kt`가 이미 `primaryPackage`로 특정 앱 하나를
채워서 내려주고 있었음): `detail(i)`를 실제 문장으로 재작성 --
예) "5초컷" → "'카카오톡' 앱을 7번이나 짧게 열었다가 바로 닫았어요".
한국어 조사(을/를) 문제는 앱 이름이 아니라 고정 단어 "앱"에 붙게
문장을 짜서 회피(예: "'{앱}' 앱을 ~" -- 조사는 항상 "앱"에 붙음,
실제 세계 아무 앱 이름에나 안전하게 적용됨).

**2. 앱 이름이 아예 없던 6개 타입** (PATROL/APP_WANDERING/
NIGHT_PATROL/DIGITAL_LOST/DAWN_SURVIVOR/HIDDEN_NIGHT_ACTIVITY --
"여러 앱을 오간" 세션 단위 사건이라 `primaryPackage`가 원래
`null`이었음): `core`(Kotlin) 쪽에 새 필드 `DetectedIncident.topPackage`
를 추가 -- 그 사건이 벌어진 세션/구간 안에서 실제로 가장 많이 방문한
앱 하나를 뽑아서(`mostVisitedPackage()`) 설명 문장에만 씀. **중요**:
`resolve()`의 하루-당-사건-개수 중복제거는 여전히 `primaryPackage`
(변경 안 함) 기준이라 이 필드는 그날 카드가 몇 개 뜨는지, 등급 배분이
어떻게 되는지에 전혀 영향을 안 줌 -- 순수하게 설명 텍스트만 좋아짐.
예) "심야 순찰" → "심야에 '카카오톡' 앱 등 8개 앱을 오가며 35분 동안
깨어있었어요".

**3. HIDDEN_LOOP** (정확히 두 앱이 번갈아 나오는 사건인데 지금까지
"두 앱 사이 전환 8회"처럼 어느 두 앱인지 전혀 안 알려줬던, 가장
눈에 띄는 정보 누락 사례): 새 필드 `secondaryPackage` 추가, 감지
시점에 `primaryPackage`=첫 번째 앱, `secondaryPackage`=두 번째 앱으로
채움. 예) "무한루프" → "'인스타그램' 앱과 '유튜브' 앱 사이를 8번이나
오갔어요". (`primaryPackage`가 `null`에서 실제 앱 이름으로 바뀌는 게
`resolve()`의 그룹핑 키에 영향은 주지만, HIDDEN 등급은 하루 최대
1장으로 이미 캡이 걸려 있어서 -- v0.61 참고 -- 최종적으로 뜨는 카드
개수에는 아무 변화 없음을 별도 JVM 검증 스크립트로 확인.)

두 신규 필드(`topPackage`/`secondaryPackage`) 모두 표시 전용, 기본값
`null`이라 기존 저장된 백업/기록 형식과도 호환. `NativeBridge.kt`의
`incidentToJson()`에 두 필드 JSON 직렬화 추가.

앱 이름/사건 데이터가 아예 없는 예외 상황(예: 방문 목록이 비어
`mostVisitedPackage()`가 `null`을 반환하는 극단적인 경우)을 대비해
14개 타입 전부 "이름 없이도 뜻이 통하는" 기존 문구로 폴백하도록
`detail(i)`를 작성 -- 즉 이번 변경은 순수 추가(additive)이지 예외
상황을 깨는 변경이 아님.

버전 66/0.66.0. 검증: (a) `core` 모듈은 JVM 전용 하네스(기존
DetectorSmoke/DetectorRegression 재실행 -- 둘 다 그대로 통과, 필드
추가가 기존 로직/테스트에 영향 없음 확인)에 더해, PATROL이 실제로
가장 많이 방문한 앱을 `topPackage`로 뽑는지, HIDDEN_LOOP이 두 앱
이름을 정확히 채우는지 확인하는 새 검증 스크립트(`VerifyTopPkg.kt`)
작성해서 통과 확인. (b) JS 쪽은 Playwright로 14개 타입 전부(+ 이름이
없는 2가지 폴백 케이스) `detail()` 출력이 비어있지 않고 앱 이름이
들어가는 것 확인, 홈 탭 피처드 카드/사건 상세 페이지 스크린샷으로
실제 렌더링 확인(피처드 카드의 `.detail`은 여러 줄로 자연스럽게
줄바꿈, 상세 페이지의 한 줄짜리 `.detail-sub`도 안 깨지고 표시됨),
기록 리스트(`.case-row-text span`)처럼 원래도 한 줄로 말줄임(ellipsis)
처리되는 자리는 긴 문장이 "..."로 잘리지만 앱 이름이 문장 맨 앞
가까이 오도록 짜서 잘려도 어떤 앱인지는 보이게 함 -- 이건 UI
구조상 원래도 있던 한 줄 제한이라 이번에 새로 생긴 문제는 아님. CSS
...
## v0.68 — 오늘의 사건 리빌 카드 애니메이션 미동작 (실기기, 원인 후보 수정)
감독 피드백(실기기): "앱 키고 오늘의 사건 나올때 그 카드는 안
움직이던데 왜 그런거야?" -- v0.60에서 만든 "카드가 휘리릭 돌아가면서"
리빌 연출(`revealScreen()`의 `.tcg-card.reveal-anim`,
`@keyframes revealCardIn`: 작고 뒤집힌 상태 -> 커지면서 정면으로)이
실기기에서 안 움직이고 카드가 정지해 있었다는 리포트.

Playwright로 `.tcg-card`의 computed transform 값을 리빌이 뜬 직후부터
80ms 간격으로 찍어봤더니, 이 샌드박스의 헤드리스 크로미움에서는
애니메이션 자체가 매 프레임 정상적으로 진행됨(회전+축소된
`matrix3d(...)` 값에서 `matrix(1,0,0,1,0,0)`(정면/원래 크기)까지
부드럽게 보간) -- 즉 keyframe 정의나 클래스 부착 로직 자체가
망가진 건 아님.

다만 코드에서 실제로 있을 법한 원인 하나는 찾음: `.tcg-card` 기본
규칙에 `transition:transform .5s`가 걸려 있는데(드래그로 카드를
기울이는 v0.54 기능이 이 transition으로 손을 뗐을 때 자연스럽게
평평해지도록 함), `.reveal-anim`이 추가하는 `animation:revealCardIn`도
같은 `transform` 속성을 건드림 -- 정확히 이 클래스의 충돌 때문에
이미 `.tcg-card.tilting{transition:none}`으로 한 번 우회해놓은
전례가 있는 문제 유형인데, `.reveal-anim`에는 그 우회가 빠져 있었음.
스펙상으로는 animation이 transition보다 우선해야 하고 이 샌드박스의
크로미움에서는 실제로 문제없이 재현됐지만, 실기기의 특정 WebView
빌드에서 이 조합을 다르게 처리해 애니메이션이 얼려 보일 가능성은
남아있어 `.reveal-anim`에도 `transition:none`을 추가(기존 `.tilting`
패턴과 동일). 위험 없는 방어적 수정 -- 이 샌드박스에서는 적용 전후
차이가 없었으므로(둘 다 정상 재생), 이게 실기기 증상의 진짜 원인인지는
다음 실기기 확인에서 재확인 필요.

버전 68/0.68.0. 검증: Playwright로 애니메이션 진행 중 여러 시점의
computed transform을 샘플링해서 수정 전/후 모두 매끄럽게 재생됨을
확인(고정/멈춤 없음), CSS 룰 개수 재확인(213개, 변동 없음), 기존
회귀 스위트(뒤로가기/온보딩/리빌/카드 3종/드래그/백업 스키마/메트릭
라벨) 전부 재확인, 콘솔 에러 0건. 실기기에서 정말 이 원인이었는지는
다음 확인 때 봐야 함 -- 만약 이 수정 후에도 그대로라면 실제
원인은 다른 곳(예: 안드로이드 시스템의 "애니메이션 제거" 접근성
설정이 이 WebView 빌드에서 실제로 CSS 애니메이션까지 억제하는
경우 등)일 수 있어 director가 리빌 화면을 볼 때 정확히 어떻게
보였는지(카드가 처음부터 완성된 크기로 바로 나타났는지, 뒤집힌
채로 멈춰 있었는지 등) 추가로 확인이 필요.

## v0.69 — 종료 확인 모달의 광고를 전면광고에서 배너로 정정 (감독 지시)
감독 피드백: "광고 제거에 관해서 얘기하고싶은데 스마트폰 물리버튼이랑
겹치는것과 종료할때 광고 띄어달라는게 아니라 종료 메시지 모달에 탭
광고를 놔두면 좋겠다는 뜻이였음". v0.65에서 "뒤로가기 종료 확인
화면에도 광고를 넣으면 좋겠다"는 요청을 "종료" 버튼을 누르는 순간
전면(interstitial) 광고를 띄우는 것으로 구현했는데, 실제로는 그런
뜻이 아니었음 -- (1) 물리 뒤로가기 버튼으로 뜨는 전면광고가 폰의
물리 버튼/제스처 영역과 시각적으로 겹치는 문제가 있었고, (2)애초에
원했던 건 "종료할 때 광고가 뜨는 것"이 아니라 "종료 확인 모달
자체에 배너 광고가 얹혀 있는 것" — 기록/보관함 탭처럼 항상 떠 있는
배너를 이 모달에도 확장해달라는 뜻이었음.

**수정 내용**:
- `NativeBridge.exitApp()`이 전면광고를 거치지 않고 바로
  `activity.finish()`하도록 원복.
- `AdManager.kt`에서 인터스티셜 관련 코드(로드/표시, 관련 import)를
  전량 제거 -- 이 변경 후 호출하는 곳이 아예 없어지는 죽은 코드라
  남겨두지 않고 완전히 삭제. 초기화 시 매번 인터스티셜을 미리
  로드하려고 네트워크 요청을 하나 더 날리던 것도 같이 없어짐.
- `index.html`: `state.exitConfirmOpen` 플래그 신규 추가.
  `openExitConfirm()`이 이 플래그를 켜고, `closeOverlay()`가 끄도록
  하고, `syncBannerVisibility()`의 배너 노출 조건에 `state.tab`이
  기록/보관함일 때와 동일하게 OR로 추가 -- 이제 종료 확인 모달이
  열려 있는 동안은 (어느 탭에서 열렸든) 항상 기존 배너가 그 아래에
  표시됨. `openExitConfirm()`/`closeOverlay()`는 둘 다 `render()`의
  일반 파이프라인을 거치지 않고 `#overlay`를 직접 조작하는 함수라
  (다른 시트 열기 함수들과 동일한 패턴), `syncBannerVisibility()`를
  두 곳에서 명시적으로 다시 호출해줘야 했음.

버전 69/0.69.0. 검증: Playwright로 (a) 홈 탭에서 종료 확인 모달을
열면 배너가 켜지고, 취소로 닫으면 다시 꺼지는 것, (b) 기록 탭에서
열어도 동일하게 배너가 유지되는 것, (c) "종료" 확정 버튼이
`N.exitApp()`만 호출하고 인터스티셜 관련 훅은 전혀 안 타는 것을
확인. 기존 회귀 스위트(뒤로가기/온보딩/리빌/카드 3종/드래그/백업
...
## v0.71 — 카드 드래그 렉/멈춤 수정 (실기기 리포트, 감독 지시 "절충안으로 가자")
감독 피드백(실기기): "카드움직일때 렉걸려서 가끔 안움직일때가
있는데 그건 왜그럴까?" 코드를 뜯어본 결과 원인은 `createCardHolo()`
(사건 상세 페이지에서 카드를 드래그하면 기울어지는 홀로그램
반짝임 효과)가 **매 프레임(초당 60번) 캔버스 전체를 처음부터
다시 그리고 있었던 것** — 그라디언트 2회 생성·전체 채우기,
레이어 합성 3회, 그리고 화면 크기에 비례해 최대 수천 개까지
늘어나는 반짝이(flake) 개별 위치·밝기 계산까지 전부 매 프레임
반복. 캔버스 해상도도 실제 폰의 `devicePixelRatio`(보통 2.5~3배)를
그대로 반영해서, 실기기 기준 카드 한 장에 flake 약 2,400~4,500개를
초당 60번 재계산하는 구조였음. 이 샌드박스(헤드리스 크로미움)는
발열/스로틀링 같은 실기기 부하 변수가 없어서 직접 재현은 못 했지만,
같은 알고리즘을 그대로 가져온 별도 벤치마크 페이지로 측정한 결과
평범한 데스크톱 브라우저에서도 프레임당 10ms 안팎(16.6ms 예산의
60%대)이 나와, 실기기의 더 느린 CPU나 발열 스로틀링이 끼면 예산을
넘기고도 남는다는 걸 확인 — "가끔"이라는 표현과 정확히 들어맞는
디바이스 부하 의존적 증상.

원인을 설명하면서 감독이 "포켓몬 TCG 포켓을 다시 참고해달라 — 거긴
그런 렉이 없다"고 요청. 실제 게임의 홀로그램 효과는 대개 반짝이
패턴을 고정 텍스처로 한 번만 만들어두고 셰이더로 그 텍스처를
이동/회전만 시키는 방식(GPU 병렬 처리라 사실상 공짜)이라는 점을
설명하고, 같은 원리를 적용한 완전 경량화 버전(고정 텍스처 +
`transform`만 이동)을 별도 Playwright 데모 페이지로 만들어
실시간 프레임 비용/버벅임 비율을 눈으로 비교할 수 있게 제공.
감독이 직접 드래그해보고 "지금 방식이 더 예쁜데 살짝 과하고,
안 만졌는데도 빛이 도는 느낌은 별로다"라는 피드백을 줌 — 시각적
완성도는 지금 방식을 선호하되 (1) 연산 비용은 줄이고 (2) 드래그하지
않을 때 자동으로 빛이 도는 연출(오토 스윕)은 없앴으면 좋겠다는
뜻으로 파악. 데모 페이지에 원본과 동일한 반짝임 알고리즘을 쓰면서
예산만 줄인 "절충안"을 세 번째 옵션으로 추가해서 다시 비교 제공
(드래그 중 평균 비용: 지금 방식 10.6ms → 절충안 2.4ms, 저사양
10배 시뮬레이션 시 지금 방식 101.8ms/버벅임 100% → 절충안
23.2ms/버벅임 50%). 감독이 "절충안으로 가자"로 최종 확정.

**실제 앱에 반영한 3가지 조치** (모두 `createCardHolo()`/`mountHolo()`,
알고리즘 자체는 그대로 유지):
1. **해상도 캡** — 캔버스 해상도를 실제 `devicePixelRatio`(2.5~3배)
   그대로 쓰지 않고 최대 2배로 제한. 픽셀 수가 최대 약 44%까지
   줄어들어 캔버스 작업(clear/그라디언트/합성) 비용이 그만큼 감소.
   반짝이 효과는 원래 선명한 디테일이 없는 층이라 화질 손실은
   체감상 없음.
2. **반짝이 밀도 축소** — flake 개수 공식을 `W*H/380`에서
   `W*H/650`으로 낮춰 면적당 개수를 약 절반으로.
3. **드래그할 때만 다시 그리고, 2프레임에 한 번만** — 기존에는
   드래그 여부와 무관하게 항상 `requestAnimationFrame` 루프가 돌면서
   안 만지고 있을 때도 자동으로 빛이 도는(오토 스윕) 연출을 계속
   렌더링하고 있었음. 이제는 드래그가 시작될 때만 렌더 루프를
   켜고, 드래그가 끝나면 즉시 끄면서 중립 포즈로 한 번만 다시
   그려서 정지 상태로 둠(`effect.render(0,0,0)`) — 감독이 지적한
   "안 만졌는데도 빛이 도는 느낌"이 완전히 사라지고, 카드가 화면에
   떠 있는 시간 대부분(드래그하지 않는 시간)의 비용이 사실상
   0이 됨. 드래그 중에도 매 프레임이 아니라 2프레임에 한 번만
   다시 그리도록 해서 남은 비용을 절반 더 줄임 — 손가락으로
   기울이는 정도의 상호작용은 30fps로도 충분히 부드럽게 느껴짐.

버전 71/0.71.0. 검증: (1) 실제 프로덕션 코드를 대상으로 한
Playwright 테스트로 드래그하지 않는 30프레임 동안 홀로그램 렌더
루프(`holoRaf`)가 단 한 번도 켜지지 않는 것(자동 스윕 완전 제거
확인), 드래그를 시작하면 루프가 켜지고 드래그가 끝나면 즉시
꺼지는 것을 확인. (2) 기존 회귀 스위트 전체(뒤로가기/메트릭
라벨/온보딩 체인/리빌/카드 3종/드래그·기울임 3종/백업 스키마)
재실행 — 전부 통과, 콘솔 에러 0건. (3) 별도 벤치마크 페이지로
같은 알고리즘의 드래그 중 평균 프레임 비용이 눈에 띄게 줄어드는
것을 측정 확인(위 수치 참고) — 다만 이 샌드박스는 실기기의
발열/스로틀링을 완벽히 재현하지는 못하므로, 실제로 "가끔 안
움직이는" 증상이 충분히 줄었는지는 다음 실기기 확인이 필요.

## v0.72 — 참여 유도 알림 (오늘의 카드 + 여러 사건 발생, 감독 지시)
감독 피드백: "1. 알림으로 오늘의 카드를 알려주기 2. 알림으로 여러
사건 발생한것을 알려주기 3. 유저가 앱을 열어보도록 유도하는
기능들이 추가되어야 됨 4. 아마 알림 아이콘도 필요하겟지..?"

기존 상태부터 점검: 이미 "일일 리마인더"라는 설정(v0.47/v0.51)이
있었지만 알림 내용이 항상 똑같은 뻔한 문구("오늘의 당신의 사건,
아직 확인 안 하셨죠?")였음 — 실제로 오늘 무슨 사건이 있었는지는
전혀 모른 채 그냥 시간 되면 울리는 알림이었던 것. 원인은 이
앱이 지금까지 일부러 "앱을 열었을 때만" 사용 기록을 분석하도록
설계돼 있었기 때문(코드 주석에도 "일부러 백그라운드 서비스
없음"이라고 명시돼 있었음) — 감독의 요청 1/2번을 구현하려면 이
앱 역사상 처음으로 백그라운드에서 스스로 분석하는 기능이
필요하다는 점을 먼저 설명하고, "1번(오늘의 카드)/2번(여러 사건)을
한 번에 다 구현" + "2번의 백그라운드 체크 주기는 3~4시간마다"로
감독의 결정을 받아 진행.

**신규 아키텍처**: `NativeBridge.analyzeToday()`가 하던 실제 분석
파이프라인(사용 이벤트 수집 → 세션 구성 → 사건 탐지 → Room
기록 → 리포트/요약 생성)을 새 `data/IncidentAnalyzer.kt`로 분리
— `Context`만 있으면 되는 순수 로직이라(액티비티 불필요) 포그라운드
(WebView가 호출)와 백그라운드(새 워커) 양쪽에서 완전히 같은 코드를
재사용, 둘이 따로 관리되다 어긋나는 일이 없음. `NativeBridge`는
이제 이 분석기를 호출하고 JS용 JSON만 조립.

이 앱 최초의 백그라운드 작업으로 `androidx.work:work-runtime-ktx`
도입(신규 `IncidentCheckScheduler.kt`가 4시간 주기
`PeriodicWorkRequest` 등록/해제, 배터리 부족 시 실행 안 하도록
`setRequiresBatteryNotLow` 제약 추가). 기존 "일일 리마인더" 알람은
계속 `AlarmManager`(재부팅 시 `BootReceiver`가 재등록) 그대로 두고
— 이건 정확히 하루 한 번이면 충분한 용도라 그대로가 더 단순함 —
새 주기적 배경 체크만 `WorkManager`로 만듦(주기적 CPU 작업을
Doze/절전 모드와 맞춰 올바르게 스케줄링해주는 것이 원래
WorkManager의 존재 이유). WorkManager는 자체 DB로 재부팅 후에도
스스로 복구되므로 `BootReceiver` 쪽은 손댈 필요 없었음. 별도
설정 UI를 새로 만드는 대신, 기존 "일일 리마인더" 토글 하나가
저녁 알림과 새 배경 체크를 함께 켜고 끄도록 연결(사용자 입장에서는
"이 앱 알림을 받을지 말지" 하나의 결정이라 토글을 두 개로
나누면 오히려 헷갈릴 것으로 판단) — 대신 리마인더 설정 시트의
설명 문구를 "이 시간에 오늘의 사건을 알려드리고, 하루 중 사건이
여러 건 쌓이면 따로도 알려드려요"로 갱신해서 무엇을 켜는 건지
투명하게 안내.

**신규 `IncidentCheckWorker.kt`**: 4시간마다 `IncidentAnalyzer`로
당일 전체를 다시 분석(사용권한 없거나 알림이 꺼져 있으면 그냥
종료, 재시도 대상 실패로 취급 안 함) — `detectDay()`가 매번
자정부터 지금까지 전체를 다시 스캔하는 기존 설계 덕분에 하루에
여러 번 호출해도 항상 최신 전체 그림이라 안전. 오늘 이미 알려준
사건 수보다 늘어났고 2건 이상이면 "오늘 벌써 N건의 사건이
발생했어요. 확인해보세요." 알림을 새로 띄움(SharedPreferences에
날짜+마지막 알림 개수 기록해서 같은 개수로는 중복 알림 안 함,
날짜 바뀌면 자동 리셋).

**"오늘의 카드" 알림**: 사건 이름을 자연어로 보여주려면 어느 앱
사건인지 이름표가 필요한데, 그 이름표(`names` 맵)는 지금까지
`index.html`의 JS에만 있었고 알림은 WebView가 꺼진 채로 울리므로
JS에 접근할 방법이 없음. `IncidentAnalyzer.analyzeToday()`가 매번
그날의 대표 사건(`report.representative`, 이미 점수/등급/길이
기준으로 가장 눈에 띄는 걸 고르도록 돼 있던 기존 로직)을
SharedPreferences에 (그날 날짜, 사건 타입, 등급) 형태로 남기고,
`DailyReminderReceiver`가 그 값을 읽어 "오늘의 사건: 무한루프
(HIDDEN 등급) — 확인해보세요!" 같은 실제 문구를 조립하도록 변경
(오늘 기록된 게 없으면 기존 일반 문구로 폴백). `index.html`의
`names` 맵을 그대로 옮긴 작은 한/일 번역 테이블을
`DailyReminderReceiver.kt`에 새로 둠 — WebView 없이 네이티브에서
울리는 알림이라 공유 소스가 없어 수동 복제, 새 사건 타입이
추가되면 두 곳 다 갱신해야 함.

**알림 아이콘**: 지금까지 `DailyReminderReceiver`가 런처
아이콘(다색 탐정 고양이 일러스트) 그대로를 상태바 아이콘으로
쓰고 있었음 — 안드로이드는 알림 아이콘을 무조건 단색 실루엣으로
강제 변환하기 때문에 복잡한 일러스트는 알아볼 수 없는 흰 덩어리로
보임(코드 주석에도 이미 "제대로 된 모노크롬 아이콘 없음"이라고
적혀 있던 상태). 새 자산을 따로 요청하는 대신 발바닥 무늬(패드
하나 + 발가락 4개, 전부 단순한 타원)로 직접 그린
`res/drawable/ic_notification.xml` 벡터 드로어블 추가 — 24dp
크기에서도 또렷하게 발바닥으로 읽힘(Playwright로 실제 크기
렌더링해서 눈으로 확인). `DailyReminderReceiver`/`IncidentCheckWorker`
둘 다 이 아이콘으로 교체. XML 주석 작성 중 이 프로젝트에서 이미
여러 번 있었던 것과 같은 실수(문장 안의 " -- "가 XML 주석을
조기 종료시키는 문제)를 또 만들 뻔했는데, 이번에도 `ET.parse()`
검증으로 즉시 잡아서 전부 "—"(em dash)로 교체.

버전 72/0.72.0. 검증: (1) 신규/수정 Kotlin 파일 전체(주석 제외
중괄호/괄호 균형) 확인. (2) `ic_notification.xml`을 `ET.parse()`로
XML 파싱 검증 + Playwright로 실제 SVG 렌더링해서 24×24 크기에서도
발바닥으로 읽히는지 육안 확인. (3) 기존 Playwright 회귀 스위트
전체(뒤로가기/메트릭 라벨/온보딩 체인/리빌/카드 3종/드래그
3종/백업 스키마) 재실행 — 전부 통과, 콘솔 에러 0건. (4) 리마인더
설정 시트를 열어 새 안내 문구가 정확히 렌더링되는지 확인. Kotlin
쪽(`IncidentAnalyzer`/`IncidentCheckWorker`/`IncidentCheckScheduler`/
`DailyReminderReceiver`/`NativeBridge`)은 이 프로젝트 최초의
`WorkManager` 사용이라 로컬 컴파일 검증이 안 되는 새 위험 영역 —
실제 컴파일 성공 여부는 CI(`build-opened-again-gradle.yml`)로만
확인 가능. `IncidentCheckWorker`가 실제로 백그라운드에서 몇 시간
간격으로 잘 발동하는지, "여러 사건 발생" 알림이 배터리 최적화가
공격적인 제조사(삼성/샤오미 등)에서도 안정적으로 오는지는 이
샌드박스에서 확인 불가 — 다음 실기기 확인 때 반드시 봐야 할
항목.

## v0.73 — 배너 광고가 물리 내비게이션 버튼과 겹치는 문제 수정 (실기기 리포트)
감독 피드백(실기기): "광고건은 내가 앱종료 정말종료하시겠습니까?
에 광고 넣어달라고 했고 기록보관함탭 광고가 스마트폰 물리버튼이랑
겹친다고 말하지않았니?" — v0.69에서 종료 확인 모달에 배너를
넣고 물리 버튼과 겹치는 문제(당시엔 전면광고 얘기)를 고쳤다고
생각했는데, 실기기에서 보니 여전히 해결이 안 됐다는 지적.

먼저 리빌 화면 카드가 손으로 안 움직인다는 별도 리포트("오늘의
사건 발견! 화면 카드 역시 안움직이는데")도 같이 들어와서 확인
— 이건 버그가 아니라 원래 설계대로였음: 홀로그램 반짝임+드래그
기울이기 효과는 EPIC/LEGENDARY/HIDDEN 등급 카드에만 붙어있고
(사건 상세 페이지도 동일), NORMAL/RARE 등급은 캔버스 자체가
없어 원래 정적. 감독이 "에픽이상만"으로 확답 — 현재 동작 유지,
수정 사항 없음.

광고 겹침 문제는 실제 원인 발견: `MainActivity.kt`가 안드로이드
15+(이 앱의 targetSdk)에서 `setDecorFitsSystemWindows(false)`로
엣지투엣지를 켜는데(v0.38), 배너 광고를 담는 네이티브
`bannerContainer`(WebView 밖에 있는 별도 안드로이드 View)에는
하단 시스템 바 inset을 피하는 처리가 전혀 없었음. `index.html`의
`.app` CSS는 `env(safe-area-inset-bottom)`으로 WebView **자기 자신의**
콘텐츠는 안전하게 처리하고 있었지만, 그건 WebView 내부 렌더링에만
적용되는 것이라 WebView 바깥의 네이티브 배너 뷰에는 아무 도움이
안 됐던 것. 그 결과 안드로이드 15+ 실기기(제스처 내비게이션 또는
3버튼 내비게이션)에서는 배너 광고가 하단 내비게이션 바/제스처
영역에 반쯤 가려지거나 완전히 그 뒤에 렌더링되고 있었음 — 이
배너는 기록/보관함 탭과 (v0.69부터) 종료 확인 모달 양쪽에서 다
쓰이는 같은 뷰라서, 감독이 지적한 두 가지 문제("종료 확인 모달의
광고"와 "기록/보관함 탭 광고가 물리 버튼과 겹침")가 사실 같은
근본 원인 하나였던 것으로 확인됨.

**수정**: `root`(WebView + bannerContainer를 담은 최상위
LinearLayout)에 `ViewCompat.setOnApplyWindowInsetsListener`를
달아서, 시스템 바(`WindowInsetsCompat.Type.systemBars()`)의 하단
inset만큼 `bannerContainer`에 하단 패딩을 줌 — 이제 배너가 내비게이션
바/제스처 영역 위쪽에 안전하게 뜸. 안드로이드 15 미만 기기에서는
시스템이 이미 내비게이션 바를 위한 공간을 앱 밖에 따로 확보해주므로
전달되는 inset이 0이라 이 코드가 그냥 아무 일도 안 함(무해). 리스너를
`bannerContainer` 자체가 아니라 `root`에 붙인 이유: `bannerContainer`는
시작 시 `View.GONE` 상태라 일부 안드로이드 버전에서 최초 inset
전달을 못 받을 수 있어서, 항상 보이는 `root`에서 받아 `bannerContainer`에
패딩만 적용하도록 함.

버전 73/0.73.0. 검증: (1) 신규/수정 Kotlin 파일 주석 제외 중괄호/괄호
균형 확인 — `MainActivity.kt`는 이전부터 있던 특정 문자열 이스케이프
패턴 때문에 이 프로젝트의 단순 괄호 카운팅 스크립트 자체가 원래도
불일치를 내는 파일이었음을 직전 커밋(v0.72, 이미 CI 통과)과 비교해서
확인 — 내가 추가한 코드 블록 자체는 육안 검토로 괄호가 맞음. (2)
기존 Playwright 회귀 스위트 핵심 항목(뒤로가기/리빌/카드 3종/백업
스키마) 재실행 — 이번 수정은 순수 네이티브 레이아웃 변경이라 JS
쪽 영향은 없지만 안전차 재확인, 전부 통과. (3) `WindowInsetsCompat`
API 자체는 실제 안드로이드 런타임에서만 진짜로 검증되는 부분이라
(이 샌드박스는 안드로이드 SDK/에뮬레이터 없음) 최종 확인은
CI 컴파일 통과 + 다음 실기기 확인 필요.

## v0.74 — 앱 아이콘 전면 교체 (감독 제공 최종 에셋)
감독이 "MONI_app_icon_final_pack.zip" 파일을 전달하며 "앱아이콘이야"로
새 앱 아이콘 적용 지시. 기존 아이콘(연한 하늘색 배경 + 심플한 고양이,
`art/ADAPTIVE_ICON_SPEC.md` v1)에서 탐정 컨셉을 훨씬 진하게 살린
새 디자인(짙은 인디고/퍼플 배경, 셜록 모자를 쓴 MONI가 폰과 돋보기를
들고 있고, 주변에 인터넷/메시지 아이콘과 연결선 장식)으로 전면 교체.

전달받은 팩(`art/icon_pack_2026-09-17/`에 원본 보관) 구성: 1024px
마스터 아이콘, "안전 플랫" 버전(OEM 마스크 크롭을 고려해 구도를
안쪽으로 당긴 버전), 배경 전용 그라디언트 레이어, Play 스토어
등록용 512px, 소형 가독성 미리보기, 원형/라운드/스쿼클 마스크
크롭 미리보기, 그리고 앱에 넣으면 안 되는 안전영역 가이드 이미지.
README에 이미 "이건 진짜 분리형 foreground/background XML이
아니라 안전영역 검토용/플랫 후보"라고 명시돼 있어서, 이 팩에는
투명 배경의 순수 캐릭터 컷아웃이 없다는 걸 확인 — 그래서
"안전 플랫" 이미지(캐릭터+배경이 이미 합쳐진 완전 불투명 이미지)를
`ic_launcher_foreground`로, 별도 그라디언트를 `ic_launcher_background`로
매핑. 포그라운드가 완전 불투명이라 평상시엔 배경 레이어를 완전히
덮어버리지만, 일부 런처가 아이콘 눌림/시차 애니메이션 중 아주
잠깐 배경만 보여주는 경우가 있어서 단색 대신 같은 톤의 그라디언트를
넣어둠. 레거시 `ic_launcher`/`ic_launcher_round`(API 26 미만 전용
폴백)는 1024px 마스터 아이콘에서 직접 리사이즈(round는 원형
알파 마스크 추가) — 이 앱은 `minSdk 29`라 이 두 파일은 실제
기기에서는 절대 안 보이지만(항상 어댑티브 아이콘 경로로 감),
툴체인 완결성을 위해 그대로 생성.

전 밀도(mdpi/hdpi/xhdpi/xxhdpi/xxxhdpi, 어댑티브 108~432px·레거시
48~192px) 전부 Pillow로 고품질 리샘플링해서 일괄 재생성 — 총 20개
PNG 파일 교체. `art/ADAPTIVE_ICON_SPEC.md`에 새 팩(v2)의 출처,
파일 매핑, "진짜 레이어드 아이콘이 아니다"라는 제약을 문서화하고
기존 v1 스펙은 "superseded"로 표시해 남겨둠. Play 스토어 등록용
512px 아이콘은 앱 리소스에는 안 들어가고 나중에 Play Console
스토어 등록 화면에 직접 업로드해야 하는 것이라 별도로 안내.

버전 74/0.74.0. 검증: 배경+포그라운드를 실제로 합성한 뒤 정사각형/
원형/둥근 사각형 세 가지 마스크를 전부 씌워서 렌더링 — 가장
빡빡하게 잘리는 원형 마스크에서도 돋보기/모자/귀 등 핵심 요소가
전혀 잘리지 않는 것을 육안으로 확인(팩 자체가 이미 이 안전영역을
고려해서 만들어졌다고 설명돼 있던 것과 일치). 레거시 아이콘도
동일하게 렌더링 확인. 전부 밀도의 파일 크기/모드(RGBA)/픽셀
크기가 기존 규격과 정확히 일치하는지 확인. 이건 리소스(PNG)만
바뀐 변경이라 Kotlin/JS 코드는 전혀 안 건드림 — 코드 쪽 회귀
위험은 없음, 실제 아이콘이 기기 런처에서 어떻게 보이는지는
다음 실기기 확인 때 봐야 함(특히 이 세션에서 검증한 원형 마스크
외에 삼성 원UI 등 실제 OEM 스킨의 자체 마스크 모양에서도 잘리는
곳 없는지).

## v0.75 — 광고 위 빈 여백 + 리빌 카드 드래그 완전 미동작 수정 (실기기 리포트, 스크린샷 3장)
감독이 실기기 스크린샷 3장과 함께 두 가지 리포트: "광고텝위에
여백이 너무많음 앱종료 모달에넣어달라고했는데 반영안됨" /
"오늘의사건 보여줄때 탭으로 움직여도 카드안움직임".

**① 광고 위 여백**: v0.73에서 배너 광고 컨테이너에 하단 시스템 바
inset만큼 패딩을 추가했는데, 실기기 스크린샷을 보니 WebView
콘텐츠(하단 탭바)와 광고 사이에 앱 배경색의 빈 공간이 생겨
있었음. 원인: 이 WebView는 `LinearLayout`에서
`layout_height=0dp + weight=1`로 옆의 `bannerContainer`와 공간을
나눠 쓰는 구조인데, **안드로이드 WebView가 형제 뷰의 크기/표시
상태가 바뀔 때 자기 자신의 렌더링을 항상 즉시 다시 반영하지는
않는 알려진 동작**이 있음 — 안드로이드 레이아웃 자체는 새 크기를
정확히 계산하지만, WebView 내부 컴포지터가 그 변화를 스스로
알아채지 못하는 경우가 있음. `AdManager.kt`에 `attachWebView()`를
추가해서 `MainActivity`가 생성한 WebView 참조를 넘겨주고,
`setBannerVisible()`/`onAdsRemoved()`가 광고 표시 상태를 바꿀 때마다
`webView.requestLayout()`을 명시적으로 호출하도록 수정. v0.73에서
추가한 시스템 바 inset 리스너도 패딩을 바꿀 때마다 같은 방식으로
`webView.requestLayout()`을 호출하도록 보강(회전이나 내비게이션
모드 전환으로 inset이 다시 전달될 때도 커버).

**② 리빌 카드 드래그 완전 미동작 (실제 버그, v0.68 미스터리 해결)**:
감독이 확인해준 대로 "오늘의 사건 발견!" 팝업 화면에서, 등급이
LEGENDARY(스크린샷의 "재입장 사건")인데도 카드를 눌러서 움직이는
게 전혀 안 됐다는 리포트. 실제 원인이 뭔지 추측 대신 Playwright로
직접 재현: LEGENDARY 리빌 카드를 강제로 띄우고 드래그해보니
`mountHolo()`의 드래그 로직 자체는 정상 동작해서 `cardEl.style.transform`에
올바른 회전값을 인라인으로 넣고 있었지만, **실제 화면에 계산되는
computed transform은 항상 `matrix(1,0,0,1,0,0)`(회전 없음)로 완전히
고정**돼 있는 걸 확인. 원인은 `.tcg-card.reveal-anim{animation:revealCardIn
.8s ... both}`의 `both` fill mode — 이 옵션은 애니메이션이 재생을
끝낸 뒤에도 그 요소에 계속 "붙어있는" 상태를 유지시켜서,
`transform` 속성에 대해서는 인라인 스타일보다 항상 우선권을
영원히 가져가 버림. `reveal-anim` 클래스가 등장 애니메이션이
끝난 뒤에도 전혀 제거되지 않고 그대로 남아있었던 게 진짜 원인 —
그래서 카드가 "얼어붙은" 것처럼 손으로 아무리 움직여도 반응이
없었던 것. v0.68에서 이 증상을 처음 리포트 받았을 때는
`transition`과 `animation`의 충돌로 추정하고 `transition:none`을
방어적으로 추가했었는데(당시엔 이 샌드박스에서 재현이 안 돼서
확신 없이 넘어갔던 항목), 이번에 실제 진짜 원인을 찾아서 해결—
그 방어적 수정 자체는 무해하니 그대로 둠. 상세 페이지의 카드는
애초에 `reveal-anim` 클래스를 쓴 적이 없어서 이 문제와 무관했고,
그래서 이전 v0.71 드래그 성능 작업 때는 전혀 발견되지 않았음.

**수정**: `render()`의 `state.revealItem` 분기에서, 카드 엘리먼트에
`animationend` 이벤트를 한 번(`{once:true}`) 걸어서 등장 애니메이션이
실제로 끝나는 순간 `reveal-anim` 클래스를 제거하도록 추가. 애니메이션의
마지막 키프레임 자체가 이미 `scale(1) rotateY(0deg)`(사실상 무변형)라서,
클래스를 떼어내도 화면상 아무 점프 없이 그대로 이어짐 — 그 다음부터는
`transform`이 평범한 CSS 캐스케이드 규칙(기본 `.tcg-card` transition +
`mountHolo()`의 드래그 로직)을 따르게 되어 정상적으로 반응함.

버전 75/0.75.0. 검증: (1) Playwright로 LEGENDARY 리빌 카드를 강제
생성해서 드래그 재현 — 수정 전에는 인라인 스타일은 정상인데
computed transform이 고정(`matrix(1,0,0,1,0,0)`)돼 있음을 확인,
수정 후에는 실제 회전이 반영된 `matrix3d(...)` 값으로 정상 변경되는
것 확인. (2) 기존 회귀 스위트 전체(뒤로가기/메트릭 라벨/온보딩
체인/리빌/카드 3종/드래그 3종/백업 스키마) 재실행 — 전부 통과,
콘솔 에러 0건, 리빌 카드의 정지 상태 transform 값도 시각적으로
동일(`matrix(1,0,0,1,0,0)` -> `none`, 둘 다 "변형 없음"을 뜻하는
동등한 표현이라 실제 차이 없음). (3) 광고 여백 수정은 순수 네이티브
레이아웃 타이밍 이슈라 이 샌드박스(안드로이드 런타임 없음)에서는
`requestLayout()` 호출이 실제로 그 간격을 없애는지 직접 확인은
불가 — 다음 실기기 확인에서 광고 위 여백이 사라졌는지 재확인
필요.

## v0.76 — 앱 종료 확인 모달에 네이티브 광고 카드 임베드 + 알림 권한 설정 라벨 오표시 수정 (감독 지시/리포트)

**요청 배경**: 다른 앱("바로숭")의 종료 확인 다이얼로그 스크린샷을 보내며
"이런 느낌으로 하고싶다는거임 종료할때는" — 아이콘+제목+설명+CTA 버튼이
다이얼로그 안에 카드 형태로 박혀있는 모습. v0.69까지는 "종료 확인 모달이
열려있는 동안 기존 배너 광고를 그대로 보여주는" 방식이었는데, 이번 요청은
그것과 달리 광고 자체가 모달 레이아웃 안의 한 요소처럼 보이길 원하는 것.

**왜 단순한 배너로는 안 되는가**: AdMob 정책상 진짜 클릭/노출 집계가
되는 "네이티브 광고" 형태(아이콘/제목/설명/CTA를 광고주가 각각 따로
제공하고, 퍼블리셔가 자기 디자인으로 배치)는 반드시 `NativeAdView`라는
실제 안드로이드 네이티브 뷰를 통해서만 렌더링해야 함 — HTML/CSS로
그 모양만 흉내내서 그리면 정책 위반. 이 앱의 종료 확인 모달은
`index.html`이 WebView 안에 그리는 HTML이라, "HTML 모달 안에 진짜
네이티브 뷰를 자리 맞춰 겹쳐놓는" 다리를 놔야 함 — WebView 좌표계와
네이티브 좌표계를 동기화하는 작업이 이 기능 비용의 대부분. 구현 전
감독에게 이 복잡도/비용을 설명하고 진행 여부를 확인받음 ("아니 저게
좋은거같음").

**구현**:
- `index.html`의 `openExitConfirm()`이 시트 안에 빈 자리표시자
  `<div id="exitAdSlot">`를 하나 더 그림(실제 광고 내용은 전혀 없음,
  순수 레이아웃 공간 예약용) — 렌더 다음 프레임(`requestAnimationFrame`,
  innerHTML 직후엔 아직 레이아웃 전이라 `getBoundingClientRect()`가
  0을 반환할 수 있어서)에 그 rect(WebView CSS px — 이 앱은
  `device-width, initial-scale=1` 뷰포트라 dp와 1:1)를 새 브릿지
  `N.showExitAd(x,y,w,h)`로 네이티브에 전달하는 `syncExitAd()` 추가.
  시트가 닫힐 때(`closeOverlay()`)는 `N.hideExitAd()` 호출.
- `AdManager.kt`: `MainActivity`가 WebView 위에 겹쳐 쌓은 빈
  `FrameLayout`(`nativeAdOverlay` — 광고 카드가 없을 땐 터치가 그대로
  WebView로 통과함)에, `showExitAd()`가 매번 새로 `AdLoader`로
  `NativeAd`를 로드해서 코드로 직접 만든 카드(둥근 흰 배경+테두리,
  "Ad" 라벨, 아이콘/제목/설명/CTA 버튼 — 색상은 `index.html`의
  `--ink`/`--muted`/`--line`/시트 confirm 버튼 색을 그대로 맞춤)를
  `getBoundingClientRect()`가 알려준 정확한 위치에 dp→px 환산해서
  올림. 시트가 열릴 때마다 새로 로드(캐싱/재사용 안 함 — 노출 집계가
  더 단순하고 정확함, 이 시트를 연속으로 여러 번 여는 경우는 드물어서
  비용 대비 이득이 작음). `NativeBridge.kt`에 `showExitAd`/`hideExitAd`
  두 브릿지 메서드 추가.
- v0.69의 "종료 확인 모달에도 기존 배너를 보여준다" 동작은 제거
  (`syncBannerVisibility()`에서 `exitConfirmOpen` 조건 삭제) — 이제
  이 화면의 광고는 새 네이티브 카드 하나뿐, 기록/보관함 탭의 배너는
  그대로 유지.
- 테스트용 광고 단위 ID는 구글 공식 "Native Advanced"
  샘플 ID(`ca-app-pub-3940256099942544/2247696110`) 사용 — 검색 결과
  중 하나가 이 ID를 "Native Video"용이라고 잘못 설명하는 경우도 있어서
  혼동 주의 (`.../1044960115` 쪽이 실제 Native Video용, 이 기능은
  비디오 에셋을 쓰지 않으므로 무관).

**추가로 같은 버전에 포함 — 알림 권한 설정 행이 항상 "허용 안 됨"으로
보이는 문제 (감독 리포트)**: "처음에 알림에 동의하는데 왜 설정에
들어가서 또 알림 권한을 탭해야되는걸까". 원인 확인: 설정 화면의
"알림 권한" 행(`settings()`)이 바로 아래 "사용정보 접근" 행과 달리
실제 권한 상태를 전혀 확인하지 않고 "탭해서 허용하기"를 무조건
하드코딩하고 있었음 — 온보딩에서 이미 허용했어도 설정에 들어가면
항상 "아직 허용 안 됨"처럼 보였던 것. `NativeBridge.kt`에
`hasNotificationPermission()` 추가(`NotificationManagerCompat.
areNotificationsEnabled()` 사용 — 단순 POST_NOTIFICATIONS 권한
체크가 아닌 이유는, 이 체크는 33 미만에서도 사용자가 나중에 시스템
설정에서 알림을 꺼버린 경우까지 정확히 반영하기 때문, 33 미만엔
애초에 그 권한 자체가 없어서 단순 권한 체크로는 항상 true로 고정됨).
`index.html`의 `settings()`가 "사용정보 접근" 행의 `accessLabel`과
같은 패턴으로 `notifLabel`을 계산하도록 수정.

버전 76/0.76.0. 검증: (0) Playwright로 알림 권한 브릿지 값을
true/false/브릿지 없음 세 가지로 바꿔가며 설정 화면을 리렌더 —
라벨이 각각 "허용됨"/"허용 필요 · 탭해서 허용"/"탭해서 확인"으로
정확히 바뀌는 것 확인. (1) Playwright로 `#exitAdSlot`의
`getBoundingClientRect()`가 0이 아닌 실제 rect를 반환하는지, 시트를
열 때 브릿지의 `showExitAd(x,y,w,h)`가 그 rect로 정확히 호출되는지,
취소 버튼/백드롭 탭/반복 열고 닫기 각각에서 `hideExitAd()`가
호출되는지, 브릿지가 아예 없는 순수 브라우저 프리뷰에서도 에러 없이
동작하는지 확인 — 전부 통과. (2) 기존 회귀(리빌 카드 드래그,
기록/보관함 탭 배너 표시 조건) 재실행 — 영향 없음, 전부 통과. (3) 실제
`NativeAdView` 렌더링/포지셔닝/터치는 네이티브 뷰이므로 이 샌드박스
(안드로이드 런타임 없음)에서는 직접 확인 불가 — CI의 Gradle 빌드
통과만으로 컴파일 정합성 확인, 실기기에서 카드가 자리표시자 위치에
정확히 겹치는지/CTA 클릭이 실제로 동작하는지는 다음 실기기 확인
필요.

## v0.77 — 광고 위 빈 여백, 세 번째 시도 만에 진짜 원인 발견 (감독 실기기 재확인)

**배경**: v0.73(내비게이션 바 겹침 padding)과 v0.75(`webView.requestLayout()`
명시 호출)를 거쳤는데도 "광고 배너 확인해봤는데 역시 여백이 아직도
존재하는거같음"이라는 재확인 리포트. v0.73/v0.75는 각각 실제로 존재하던
다른 버그(내비게이션 바와 배너 겹침 / WebView가 형제 뷰 크기 변화를
스스로 반영 못 하는 리플로우 타이밍 문제)를 제대로 고친 게 맞지만,
"광고 위 여백" 자체의 진짜 원인은 아니었던 것으로 보임 — CSS를 다시
찬찬히 살펴보고 세 번째 원인 후보를 발견.

**진짜 원인**: `index.html`의 `.app`/`.tabbar`/`.modal`/`.detail-page`/
`.toast` 등 여러 CSS 규칙이 전부 `env(safe-area-inset-bottom)`을
무조건 예약해서 하단 여백을 잡고 있음(내비게이션 바에 콘텐츠가 가려지지
않도록). 문제는 **안드로이드의 WindowInsets 전달 방식은 형제 뷰가 이미
화면 하단 공간을 차지하고 있어도 그만큼 줄여서 전달해주지 않는다**는
것 — `MainActivity.kt`의 `root` 레벨 insets 리스너가 시스템 바
하단 inset 값을 읽어서 `bannerContainer`에 패딩으로 적용하는데, 정확히
같은 inset 값이 WebView 쪽에도 별도로 그대로 전달됨. 그 결과 배너
광고가 실제로 화면에 보이고 있는 동안에도, WebView는 "내비게이션
바 회피용 여백"을 **한 번 더** 예약해버려서 — 진짜 배너 바로 위에
앱 배경색과 똑같은 색의 빈 공간이 생기는 것. 배너가 꺼져 있을 때는
WebView가 실제로 화면 맨 아래까지 닿으므로 이 예약이 정확히 필요한
만큼임 — 그래서 "배너 없을 때는 멀쩡한데 배너 켜지면 그 위에 여백이
생긴다"는 리포트와 정확히 들어맞음.

**수정 방식**: 안드로이드의 WindowInsets 소비/재전달 메커니즘을
WebView 한 곳에만 직접 손대는 방식(기기/OS 버전별로 동작이 미묘하게
다를 수 있어 이 샌드박스에서 검증이 특히 어려움) 대신, 네이티브 쪽이
이미 정확히 알고 있는 "지금 예약해야 할 진짜 값"(배너가 보이는 동안엔
0, 숨겨져 있을 땐 실제 inset 값)을 계산해서 `evaluateJavascript()`로
JS에 그대로 넘겨주는 방식을 택함. `AdManager.kt`에
`onSystemBarsBottomChanged(px)`(원본 inset 캐싱)와
`syncWebViewNavInset()`(배너 표시 상태와 조합해서 dp 변환 후 JS로
푸시) 추가, `setBannerVisible()`/`onAdsRemoved()`가 배너 표시 상태를
바꿀 때마다 재계산하도록 연결. `MainActivity.kt`의 기존 insets
리스너가 이 값을 `AdManager`에 전달. `index.html`에는 새
`window.setNativeNavInset(px)`가 `:root`에 CSS 커스텀 프로퍼티
`--native-nav-bottom`을 설정하고, 위 5개 CSS 규칙 전부
`env(safe-area-inset-bottom)` 대신
`var(--native-nav-bottom, env(safe-area-inset-bottom))`을 참조하도록
변경 — 네이티브가 아직 한 번도 값을 안 밀어준 초기 프레임이나 순수
브라우저 프리뷰에서는 그대로 `env()` 값으로 안전하게 폴백됨.

버전 77/0.77.0. 검증: Playwright로 `window.setNativeNavInset(0)`/`(48)`을
직접 호출해서 `.app`/`.tabbar`의 실제 computed padding-bottom이 각각
정확히 (기존값+0)과 (기존값+48px)로 바뀌는 것 확인 — CSS 커스텀
프로퍼티 오버라이드 자체는 정상 동작. 다만 이번 수정의 핵심은
"안드로이드 WindowInsets가 실제 기기에서 정확히 어떤 값으로 전달되고,
그 값이 배너 표시/숨김 시점과 올바르게 맞물려 재계산되는지"라 이
샌드박스(안드로이드 런타임 없음)에서는 근본적으로 직접 확인이 불가능함
— v0.73/v0.75도 이론상 맞는 것 같았지만 실기기에서 여백이 남아있었던
전례가 있어서, 이번에도 실기기 재확인 결과를 기다려야 확정 지을 수
