...
### 5. [v0.35에서 반영] 컴포넌트 스타일시트(목업 03번 우측 하단)
...
10종)는 오랫동안 자산만 복사해두고 미반영 상태였는데(탭바가 텍스트
기반이라 마크업 구조 변경이 필요해서 별도 작업으로 미뤄뒀음), 감독이
"밑에다 탭을 두려고 하지 않았나?"라고 재확인 → 목업을 다시 픽셀 단위로
대조해보니 탭바 위치(상단→하단)까지 어긋나 있던 걸 뒤늦게 발견 → v0.35에서
하단 탭바(홈/기록/보관함/설정 4개, 아이콘+라벨) + 신규 설정 화면 + 홈 화면
대표카드+압축리스트 구조 + 사건 카드 상세 전체화면화까지 한 번에 반영.
자세한 내용은 `docs/DEVELOPMENT_HISTORY.md` v0.35 참고.
