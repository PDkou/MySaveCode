...
>
> **[1, 2번 해결됨 — v0.23]** 같은 날 디자인팀이
> `openedagainfinalvisualassets20260909` 4-part 팩으로 회신 — 사건별 전용
> 일러스트 14종(1번)과 등급 알약 뱃지용 심볼 아이콘(2번)을 정식 반영. 상세는
> `docs/DEVELOPMENT_HISTORY.md` v0.23, 전체 매니페스트는
> `art/handoff/2026-09-09-final-visual-assets/`. **3번은 이미 코드에 있었음**
> (`index.html` 하단 `MONI · SMALL HABITS, BIG STORIES.` 푸터 — 이 문서 작성 시
> 놓치고 "미사용"이라고 잘못 적었던 것, 정정). **4번(온보딩)과 5번(컴포넌트
> 아이콘)은 같은 팩에 자산은 왔지만 아직 미반영** — 아래 "진행 방식" 참고.
...
**진행상황**: `onboarding/{ko,jp}/onboarding_01_*.png`,
`onboarding_02_*.png`(1/2, 2/2 각 언어 2장, 총 4장) 회신받아
`app/src/main/assets/visual/onboarding/`에 복사만 해둠 — **아직 화면에
연결 안 함**. 매니페스트가 "전체화면 완성 컴포지션이고 버튼까지 그림에
박혀 있으니, 실제 인터랙션이 필요하면 이 이미지를 시안으로만 쓰고 터치
컨트롤은 코드로 별도 구현하라"고 명시. 이 앱은 애초에 온보딩 플로우 자체가
없어서(권한 요청 화면이 `render()` 함수 안에 인라인으로만 있음) 이건 단순
자산 교체가 아니라 화면 전환/버튼 히트 영역/언어별 이미지 전환까지 포함한
새 기능 구현 — 카드 아트 교체와 같은 커밋에 묶지 않고 별도 작업으로 진행
예정.
...
1/2/3번은 v0.23에 바로 반영 완료. 4번(온보딩)/5번(라벨 아이콘)은 자산은 받았지만
화면 전환·마크업 구조 변경이 필요한 별도 기능 작업이라 다음 커밋으로 분리.
