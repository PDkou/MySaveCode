...
## 9. 정식 출시(프로덕션) 전환 시 체크리스트 (2026-09-13 정리)

비공개 테스트 이후 실제 프로덕션으로 넘어갈 때 확인/처리할 것들. 위
0~8번은 비공개 테스트 단계 기준이고, 여기부터는 그 다음 단계.

### 9-0. 게이트 (이게 안 풀리면 프로덕션 자체가 안 열림)

- [ ] 비공개 테스트 **12명 / 14일 연속** 요건 충족 확인 (Play Console
  대시보드에 "프로덕션 액세스 신청 가능" 표시되는지)
- [ ] 요건 충족되면 **프로덕션 액세스 신청**

### 9-1. Play Console에서 손으로 처리

- [x] **AdMob UMP(GDPR) 동의 양식 생성** — **완료** (2026-09-15).
  "동의하지 않음" 버튼이 국가별로 비어있던 게 실제 원인이었음(기본값
  그대로 전부 켜진 상태로 확인만 누르면 됨) — 예전에 추정했던 "공개
  스토어 페이지 부재"는 이번엔 원인이 아니었음. 개인정보 보호 및
  메시지 → 유럽 규정에 "게시됨" 상태로 확인됨.
- [~] **`SCHEDULE_EXACT_ALARM` 권한 사용 목적 선언** — **2026-09-15
  확인: 지금은 대기 중 아님.** 앱 콘텐츠 → "주의 필요" 탭에 아무것도
  없음("모두 확인했습니다")이라, 현재 구글이 이 권한에 대한 별도
  소명을 요구하고 있지 않음. 프로덕션 심사 중 구글이 요청하면(이메일
  또는 새 "주의 필요" 항목) 그때 아래 문구를 그대로 제출하면 됨 —
  지금 추가로 할 일 없음. 그대로 붙여넣을 수 있는 영문
  소명 문구(2026-09-14 작성):
  ...
  한국어 버전(수정용 초안 — 실제 제출은 영어 권장이지만 초안 검토는
  이걸로):

  ```
  핵심 기능: Hello, Today는 개인용 리마인더 앱으로, 앱의 존재 이유가
  사용자가 직접 고른(또는 앱이 자동으로 골라준 고정 간격/범위 내
  랜덤 날짜) 시각에 특정 사람(가족, 친구 등)에게 연락하도록 알려주는
  것 하나뿐입니다.

  정확한 알람이 필요한 이유: 이 앱의 핵심 가치는 알림이 "그날 중
  아무 때나" 또는 "OS가 처리해줄 때"가 아니라, 사용자가 의도한 시각에
  가깝게 도착하는 것입니다. SCHEDULE_EXACT_ALARM 없이는 안드로이드의
  배터리 최적화(특히 삼성 등 일부 제조사 커스텀 UI)로 인해 부정확한
  알람이 수십 분에서 몇 시간까지 지연될 수 있고, 이는 이 앱의 유일한
  기능을 직접적으로 무력화시킵니다.

  요청 방식: 이 권한은 시스템 설정 화면(ACTION_REQUEST_SCHEDULE_EXACT_ALARM)을
  통해 명시적으로 요청되며(몰래 요청하지 않음), 사용자가 앱 안에서
  직접 설정한 리마인더를 예약하는 용도로만 사용됩니다 — 그 외 다른
  어떤 기능도 정확한 알람에 의존하지 않습니다.
  ```
- [x] **데이터 보안(Data safety) 양식 최신화** — **확인 완료, 수정
  불필요** (2026-09-15). CSV 내보내기로 실제 제출된 값 확인함:
  "기기 또는 기타 ID"(광고 ID)만 수집+공유(광고 목적, AdMob)로
  체크돼 있고 나머지(사진/동영상, 오디오 파일, 연락처, 개인 정보 등)
  는 전부 미체크 상태. Play의 "수집됨" 정의가 "기기 밖으로 전송됨"
  이라서, 프로필 사진·통화 녹음 연결처럼 순수 로컬 참조만 하는
  기능은 애초에 체크 대상이 아님 — 지금 상태가 정확함. 데이터
  삭제 URL도 개인정보처리방침의 `#delete-data`로 정상 연결됨.
- [x] **스토어 등록정보 영어/일본어 전체 설명** — **완료** (2026-09-14).
  4-1/4-2 섹션에 번역 완료, Play Console의 언어별 번역 등록정보에
  붙여넣기만 하면 됨.
- [x] **`remove_ads` 상품 가격/지역 정책** — **완료/확인됨**
  (2026-09-15). Play Console에서 일본 가격이 이미 **JPY 330 (VAT
  없음, 사용 가능)**로 설정돼 있는 걸 확인 — 앱 내 `특定商取引法に
  基づく表示`의 "330円(税込)"과 정확히 일치. 코드 수정 불필요.
- [x] **특정상거래법 표시 최종 확인** — **완료** (2026-09-15).
  가격(330円) Play Console 실제 설정값과 일치 확인. 운영총괄책임자/
  이메일/소재지 공개 방식은 기존 그대로 유지(개인 개발자에게 허용된
  "청구 시 공개" 방식).
- [x] **개인정보처리방침 Artifact 공개 상태 유지 확인** — **완료**
  (2026-09-15). 공유 링크로 접근 가능한 공개 상태 확인함. 동시에
  8/27 이후 추가된 프로필 사진·통화 녹음 파일 연결 기능이 본문에
  전혀 언급되지 않은 걸 발견해서, 두 기능 다 "기기 내 선택만, 서버
  업로드 없음"으로 명시하는 문단을 한/영 버전 모두에 추가하고 최종
  업데이트 날짜를 9/15로 갱신함(SCHEDULE_EXACT_ALARM 권한 설명도
  누락돼 있어 같이 추가).

### 9-2. 코드에서 처리할 것

- [x] **"테스트 알림" 설정 항목 문구를 사용자 친화적으로 변경** —
  **완료** (2026-09-15). 라벨을 "알림 확인"/"通知確認"/"Check
  notifications"으로 교체(기능 자체는 그대로 유지, 부제목에 "테스트로
  도착해요"라고 남겨 동작 방식은 계속 알 수 있게 함).
  `testNotification()`의 toast 문구에서도 "테스트" 단어 제거.
- [ ] versionName을 "1.0.0"으로 올릴지 결정 (선택 사항, Google이 강제
  하지 않음)
- [ ] 그 외 서명키/광고 ID/권한 목록은 이미 실제 운영값으로 세팅되어
  있어 추가 코드 변경 불필요.

## 10. 나중에 할 일 (Backlog, 정식 출시와 무관 — 급하지 않음)

- [ ] **대화면/폴더블 대응** (2026-09-16 메모). Play 콘솔 프리론치
  리포트에 계속 뜨는 "다음 출시 버전을 위한 발견 항목" 3건 중
  2건이 `screenOrientation="portrait"` 고정 및 대화면 미대응과
  관련됨(광고성 경고이며 정책 위반 아님, 출시 차단 안 함 — 지금은
  그냥 두기로 결정). 다만 폴더블폰 보급이 계속 늘어날 걸 감안하면
  언젠가는 손볼 필요가 있음. 실제로 손댈 때 예상 작업:
  - `AndroidManifest.xml`의 세로 고정(`screenOrientation="portrait"`)
    해제 여부 결정
  - `index.html`은 웹뷰 기반이라 반응형 CSS로 대응 가능한 구조 —
    카드/리스트가 큰 화면에서 늘어지지 않게 max-width, 2단 레이아웃
    등 조정
