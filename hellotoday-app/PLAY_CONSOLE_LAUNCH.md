...
- [ ] **개인정보처리방침 Artifact 공개 상태 유지 확인** — 비공개로
  되돌아가 있지 않은지.

### 9-2. 코드에서 처리할 것

- [ ] **"테스트 알림" 설정 항목 문구를 사용자 친화적으로 변경** — 지금
  라벨("테스트 알림"/"テスト通知"/"Test reminder")이 개발자용 디버그
  기능처럼 보일 수 있음(기능 자체는 정식 유지, 지우지 않음). 정식
  출시 타이밍에 "알림 확인"/"通知確認"/"Check notifications" 같은 더
  사용자 친화적인 문구로 교체 예정. (`index.html`의 `settingsView()`
  테스트 알림 행 + `testNotification()`의 toast 문구)
