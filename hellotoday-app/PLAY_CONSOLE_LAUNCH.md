...
- [ ] **본인 테스트 기기를 AdMob 콘솔 → 설정 → 테스트 기기에 등록** — 아직
  안 함. 이제 진짜 광고 단위라, 등록 안 한 폰으로 반복 클릭/노출시키면
  "잘못된 트래픽"으로 계정이 정지될 수 있습니다. 실기기로 확인하기 전에
  먼저 하세요.
...
- [ ] **AdMob 콘솔에서 실제 동의 양식(GDPR) 만들기** — 아직 안 함. **위
  SDK 연동만으론 부족합니다** — AdMob 콘솔 → 개인정보 및 메시지에서
  동의 메시지를 실제로 만들어 게시해야, 그제서야 SDK가 보여줄 화면이
  생깁니다. 지금 상태로는 EEA/영국 사용자에게 아무 동의 화면도 안 뜨는
  채로 광고가 나갈 수 있어 그 지역 정책 위반입니다. 만든 뒤엔
  `ConsentDebugSettings`의 디버그 지역(geography) 오버라이드로 직접
  여행 없이도 EEA 사용자 시나리오를 테스트 기기에서 확인할 수 있습니다.
...
- **광고 자체도 실기기에서 처음 확인하는 지점입니다**: "연락했어요" 기록을
  ...
  뜨지 않는지 확인하세요. 이제 실제 광고 단위라 진짜 광고가 채워지는데,
  ⚠️ 3-1번의 테스트 기기 등록을 먼저 안 했다면 본인 폰으로 이 테스트를
  반복하지 마세요 (잘못된 트래픽으로 계정 정지 위험).
