...
- [ ] **AdMob UMP(GDPR) 동의 양식 생성 재시도** — 0-1의 3-1번에서 막혔던
...
- [ ] **`remove_ads` 상품 가격/지역 정책** — 지금 ₩2,900 국내 기준.
  ...
  (일본 거주/과세 관련 논의와 연결됨). 추천: **Google Play의 자동
  가격 환산 기능을 쓰는 것을 추천** — 1인 개발자가 국가별로 직접
  가격을 관리/유지보수하는 부담이 없고, 세금(부가세 등)도 Google이
  지역별로 알아서 반영해줌. ⚠️ 단, `특定商取引法に基づく表示`에
  이미 **"330円(税込)"**이 하드코딩돼 있는데, 이게 자동 환산 가격과
  실제로 일치하는지 Play Console에서 직접 확인 필요 — 다르면
  `index.html`의 tokushoho 문구도 같이 고쳐야 함.
- [ ] **특정상거래법 표시 최종 확인** — 실사용자가 실제로 보게 되니
  주소/연락처 등 정보가 최신/정확한지 마지막 재확인.
