...
- [ ] **갤럭시 워치(웨어러블)에 알림이 안 뜸** — 조사 중. 이 앱은 워치
  전용 코드가 없고 표준 안드로이드 알림은 자동으로 워치에 미러링되는 게
  정상이라, 앱 코드 문제가 아니라 갤럭시 웨어러블 앱의 앱별 알림 허용
  설정(또는 워치 자체 알림 차단/방해금지 동기화) 쪽 원인일 가능성이 높음.
  워치 쪽 알림 설정 확인 후 재검증 필요.
