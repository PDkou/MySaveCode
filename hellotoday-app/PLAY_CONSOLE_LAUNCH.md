...
- [ ] **데이터 보안(Data safety) 양식 최신화** — 0.4.x 이후 추가된
  프로필 사진 선택, 통화 녹음 파일 연결 기능이 반영됐는지 확인.
  실제로 폼에 체크할 값 정리(2026-09-15, 개인정보처리방침 갱신
  내용과 동일 기준):
  - **개인 정보(이름 등)**: 수집 안 함 (기기 밖으로 전송되지 않음)
  - **사진 및 동영상**: 수집 안 함 (프로필 사진, 기기 내 선택기로만
    참조, 업로드/사본 없음)
  - **오디오 파일**: 수집 안 함 (연결한 통화 녹음도 기기 내 선택기로만
    참조)
  - **앱 활동 / 앱 정보 및 성능**: 수집 안 함 (자체 분석 도구 없음)
  - **기기 또는 기타 ID(광고 ID)**: **수집함 + 공유함**, 용도는
    "광고", Google AdMob이 처리(이미 2번 섹션에 반영돼 있던 내용,
    변경 없음)
  - 이 앱은 데이터 암호화 전송 여부/삭제 요청 방법도 물어볼 텐데,
    "전송 자체가 없음"과 "설정 → 모든 데이터 삭제로 즉시 삭제 가능"
    으로 답하면 됨(개인정보처리방침과 동일 문구).
