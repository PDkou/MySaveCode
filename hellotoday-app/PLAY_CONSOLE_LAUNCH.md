...
- [ ] **광고 제거 구매 흐름 실기기 검증** — 아직. 3번째 사람 등록 시도 →
  "광고 제거" 시트 → 구매 버튼 → 실제 Play 결제 UI가 뜨는지 확인 (테스트
  트랙에서도 실제 결제가 됩니다 — 라이선스 테스터로 등록하면 청구 없이
  테스트 가능: **Play Console → 설정 → 라이선스 테스트**에서 테스터 계정을
  라이선스 테스트 계정으로도 등록해두는 걸 권장), 구매 후 광고가 더 이상
  안 뜨는지도 확인.
